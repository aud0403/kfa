...
select  b.stdr_year_nm     as   연도
       ...
       , d.crse_odr_nm      as  소집회차
       , a.item_cde         as  종목코드
       , e.item_nm          as  종목
       , a.player_id 		as  선수아이디
       , f.player_nm        as  선수명
       , concat(substring(f.brthdy,1,4),'.',substring(f.brthdy,5,2),'.',substring(f.brthdy,7,2))  as  선수생년월일
       , f.hmnm_se_nm       as  선수동명구분명
       , h.sexdstn_nm       as  선수성별
       , a.player_sexdstn_cde  		as  선수연령코드
       , i.agrde_nm         as  선수연령대
       , a.cnvc_cnter_se_cde AS 소집센터코드
       , l.cnvc_cnter_se_nm as  소집센터구분
       , a.cnvc_wdr_cde     as	소집광역코드
       , m.cnvc_wdr_nm      as  소집광역
       , a.cnvc_area_cde    as  소집지역코드
       , n.cnvc_area_nm     as  소집지역
       , a.cnvc_team_grad_cde AS 소집팀등급코드
       , o.team_grad_nm     as  소집팀등급
       ...
       , z.team_class_nm    as  소속팀클래스
       , a.psitn_team_grad_cde      as  소속팀등급코드
       , aa.team_grad_nm    as  소속팀등급
       , a.psitn_team_mangr_id 		as  소속팀감독코드
       , ab.coach_nm        as  소속팀감독명
       , concat(substring(ab.brthdy,1,4),'.',substring(ab.brthdy,5,2),'.',substring(ab.brthdy,7,2))  as  소속팀감독생년월일
       , ab.hmnm_se_nm      as  소속팀감독동명구분명
       , ac.player_postn_nm as  소속팀선수포지션
       , ad.traing_place_nm as  훈련장소명
       , a.evl_opinion_cde as  평가의견코드
       , af.evl_opinion_nm  as  평가의견
       , a.cnvc_sttus_cde  as  소집상태코드
       , ag.cnvc_sttus_nm   as  소집상태
       , a.height           as  신장
       , a.bdwgh            as  체중
       , a.cnvc_no          as  소집수
...
comment on column kfadm.골든에이지선수.선수연령코드 is '선수연령코드';
comment on column kfadm.골든에이지선수.선수연령대 is '선수연령대';
comment on column kfadm.골든에이지선수.소집센터코드 is '소집센터코드';
comment on column kfadm.골든에이지선수.소집센터구분 is '소집센터구분';
comment on column kfadm.골든에이지선수.소집광역코드 is '소집광역코드';
comment on column kfadm.골든에이지선수.소집광역 is '소집광역';
comment on column kfadm.골든에이지선수.소집지역코드 is '소집지역코드';
comment on column kfadm.골든에이지선수.소집지역 is '소집지역';
comment on column kfadm.골든에이지선수.소집팀등급코드 is '소집팀등급코드';
comment on column kfadm.골든에이지선수.소집팀등급 is '소집팀등급';
comment on column kfadm.골든에이지선수.소집성별 is '소집성별';
comment on column kfadm.골든에이지선수.사용발코드 is '사용발코드';
...
comment on column kfadm.골든에이지선수.소속팀코드 is '소속팀코드';
comment on column kfadm.골든에이지선수.소속팀명 is '소속팀명';
comment on column kfadm.골든에이지선수.소속팀지역코드 is '소속팀지역코드';
comment on column kfadm.골든에이지선수.소속팀지역 is '소속팀지역';
comment on column kfadm.골든에이지선수.소속팀클래스코드 is '소속팀클래스코드';
comment on column kfadm.골든에이지선수.소속팀클래스 is '소속팀클래스';
comment on column kfadm.골든에이지선수.소속팀등급코드 is '소속팀등급코드';
comment on column kfadm.골든에이지선수.소속팀등급 is '소속팀등급';
comment on column kfadm.골든에이지선수.소속팀감독코드 is '소속팀감독코드';
